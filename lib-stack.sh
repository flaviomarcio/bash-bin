...
function __private_stackEnvsLoadByStack()
{
  ...
  export STACK_SERVICE_STORAGE_CONFIG_DIR=${__private_stackEnvsLoadByStack_storage}/log
  ...
  stackMkDir 777 "${STACK_SERVICE_STORAGE_DATA_DIR} ${STACK_SERVICE_STORAGE_LOG_DIR}" ${STACK_SERVICE_STORAGE_CONFIG_DIR}" ${STACK_SERVICE_STORAGE_BACKUP_DIR}"
...
function __private_stackEnvsDefaultByStack()
{
  ...
  local __service=${__environment}-${__target}-${__name}
  ...
  envsSetIfIsEmpty APPLICATION_DEPLOY_VAULT_IMPORT "${STACK_VAULT_IMPORT}/${APPLICATION_DEPLOY_NAME}"
...
function stackEnvsClearByStack()
{
  ...
  unset APPLICATION_DEPLOY_BINARY_DIR
